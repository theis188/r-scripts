...
alby_q <- ("SELECT mrn, min(value) alby 
           from labs
           where lab=\"Hemoglobin\" 
           group by mrn;")

alby <- dbGetQuery(con, alby_q)

j <- merge(aa,alby)
f <- j[j$symptom==FALSE,]$alby
t <- j[j$symptom==TRUE,]$alby
boxplot(f,t)
t.test(f,t)
...
alby_q <- ("SELECT mrn, avg(value) alby 
           from labs
           where lab=\"Albumin\" 
           group by mrn;")
...
###### Fatigue (PHYSISCIAN REPORPTED) VS Hemobubin
